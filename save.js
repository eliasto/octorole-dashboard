                    ...
                    <tr key={values.id}>
                      <td className="p-2 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="font-medium text-gray-800">{values.id}</div>
                        </div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left">{values.emoji}</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium">{values.name}</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium text-green-500">{values.price} €</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium">{values.type}</div>
                      </td>
                      {(values.highlight?                       <td className="p-2 whitespace-nowrap">
                      ...
                      <td className="p-2 whitespace-nowrap">
                      <button className="ml-2" onClick={() => {setHighlightName(values.name); setHighlightIndex(index); setOpenModaleHighlight(!openModalHighlight)}}><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
</svg></button>
                        <button className="ml-2" onClick={() => {setOpen(!open); setValue(values); setValueIndex(index); updateValues(values)}}><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
<button className="ml-2" onClick={() => {setDeleteName(values.name); setDeleteIndex(index); setOpenModaleDelete(!openModalDelete)}}><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
    ...
    <Pagination count={count} setCount={setCount} currentIndex={currentIndex} setCurrentIndex={setCurrentIndex} />
    </div>  
  ...
  async function deleteProduct(index){
    await axios.delete(`${apipath}/products/${data[index].id}` ,{
        ...
        .then(res => {
          if(res.status === 200){
            toast().success('Produit supprimé', `Le produit ${formName} a bien été supprimé.`).for(3000).show() //display for 3000ms
